...
class MovingCharges(Scene):
    ...
    def construct(self):
        ...
        self.field = field
        source_charges = self.Positron().move_to(self.point_charge_loc)
        self.play(FadeIn(source_charges))
        self.play(ShowCreation(field))
        self.moving_charge()
...
class FieldOfMovingCharge(Scene):
    CONFIG = {
        ...
        "point_charge_loc": LEFT * 5.5 - UP * 1.5
    }
    ...
    def construct(self):
        ...
        field = VGroup(*[self.create_vect_field(self.point_charge_loc, x * RIGHT + y * UP)
            for x in np.arange(-9, 9, 1)
            for y in np.arange(-5, 5, 1)
        ])
        self.field = field
        self.source_charge = self.Positron().move_to(self.point_charge_loc)
    ...
    def moving_charge(self):
        ...
        possible_points = [v.get_start() for v in self.field]
        points = random.sample(possible_points, num_charges)
        particles = VGroup(self.source_charge, *[self.Positron().move_to(point) for point in points])
